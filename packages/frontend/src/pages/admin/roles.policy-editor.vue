...
<template>
	<div class="_gaps_s">
		...
		<XFolder v-if="matchQuery([i18n.ts._role._options.ignoreServerSilence, 'ignoreServerSilence'])" v-model:policyMeta="policyMetaModel.ignoreServerSilence" :isBaseRole="isBaseRole" :readonly="readonly">
			<template #label>{{ i18n.ts._role._options.ignoreServerSilence }}</template>
			<template #valueText>{{ valuesModel.ignoreServerSilence ? i18n.ts.yes : i18n.ts.no }}</template>
			<template #default="{ disabled }">
				<MkSwitch v-model="valuesModel.ignoreServerSilence" :disabled="disabled">
					<template #label>{{ i18n.ts.enable }}</template>
				</MkSwitch>
			</template>
		</XFolder>
