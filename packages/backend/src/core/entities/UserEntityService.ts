...
export class UserEntityService implements OnModuleInit {
	...
	avatarDecorations: user.avatarDecorations && user.avatarDecorations.length > 0 ? this.avatarDecorationService.getAll(false, true).then(decorations => user.avatarDecorations.filter(ud => decorations.some(d => d.id === ud.id)).map(ud => ({
		id: ud.id,
